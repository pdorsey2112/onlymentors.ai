...
const CreatorDashboard = () => {
  ...
  const fetchCreatorStats = async (creatorId) => {
    try {
      ...
      // Fetch creator stats from the dedicated endpoint
      const response = await fetch(`${backendURL}/api/creators/${creatorId}/stats`, {
        headers: {
      ...
      if (response.ok) {
        ...
        setStats(prev => ({
          ...
          premium_content_count: stats.premium_content_count || 0,
          premium_earnings: stats.premium_earnings || 0,
          total_content_sales: stats.total_content_sales || 0,
          total_premium_revenue: stats.total_premium_revenue || 0,
          total_content: stats.total_content || 0
        }));
        
        console.log('Creator stats updated:', stats);
      } else {
        console.error('Failed to fetch creator stats:', response.status, response.statusText);
      }
